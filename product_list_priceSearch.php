...
if( (isset($_POST['sprice'])) && (!empty($_POST['price1'])) && (!empty($_POST['price2'])) )
{
  $price1 = preg_replace('#[^0-9]#i','',$_POST['price1']);
    $price2 = preg_replace('#[^0-9]#i','',$_POST['price2']);
  $dynamicList = "";
  $sql = oci_parse($conn,"select * from product where price between $price1 AND $price2");
...
{
  $dynamicList = "We have no products within this price range.";  
}
}
else
{
  if( ((!empty($_POST['price1'])) || (!empty($_POST['price2']))) || ((empty($_POST['price1'])) && (empty($_POST['price2']))) )
  { $dynamicList = "Please enter the price range in correct format."; }
  else
...
<title>Product List Price Range Result</title>
